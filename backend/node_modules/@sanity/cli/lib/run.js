'use strict';

var path = require('path');
var cli = require('./_chunks/cli-9d378218.js');
require('./_chunks/resolveRootDir-56334fb5.js');
require('url');
require('events');
require('https');
require('http');
require('util');
require('get-it');
require('get-it/lib/middleware/observable');
require('get-it/lib/middleware/jsonRequest');
require('get-it/lib/middleware/jsonResponse');
require('get-it/lib/middleware/progress');
require('get-it/lib-node/middleware/retry');
require('get-it/lib-node/middleware/debug');
require('get-it/lib-node/middleware/headers');
require('fs');
require('tty');
require('os');
require('module');
require('child_process');
require('get-it/middleware');
require('pkg-dir');
require('constants');
require('stream');
require('assert');
require('buffer');
require('crypto');
require('./_chunks/getCliConfig-ddb40a5c.js');
require('worker_threads');
require('esbuild-register/dist/node');
require('readline');
require('fs/promises');
require('@babel/traverse');
require('get-it/lib-node/middleware/promise');
require('zlib');
function _interopDefaultCompat(e) {
  return e && typeof e === 'object' && 'default' in e ? e : {
    default: e
  };
}
var path__default = /*#__PURE__*/_interopDefaultCompat(path);
cli.getCliVersion().then(cliVersion => {
  cli.runCli(path__default.default.join(__dirname, ".."), {
    cliVersion
  });
});
//# sourceMappingURL=run.js.map
